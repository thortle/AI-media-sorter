...
def check_reference_dataset():
    ...
    if len(image_files) == 0:
        print(f"  ⚠️  No reference photos found in {dataset_path}")
    ...
    if len(image_files) < 10:
        print("  ⚠️  Warning: Less than 10 reference photos")
...
def check_descriptions_file():
    ...
    try:
        ...
        if photo_count == 0:
            print("  ⚠️  Warning: No photos in descriptions.json")
            return False
        
    except Exception as e:
        print(f"  ⚠️  Warning: Could not validate file: {e}")
